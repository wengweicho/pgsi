...
def cartorder(request):  #按我要結帳鈕
	...
	message1 = message
	return render(request, "cartapp/cartorder.html", locals())
...
def cartok(request):  #按確認購買鈕
	...
	if customname=='' or customphone=='' or customaddress=='' or customemail=='':
		...
		return redirect('/cartapp/cartorder/')
	else:
		...
		cartlist = []
		request.session['cartlist'] = cartlist
		return render(request, "cartapp/cartok.html", locals())
...
def cartordercheck(request):  #查詢訂單
	...
	return render(request, "cartapp/cartordercheck.html", locals())
