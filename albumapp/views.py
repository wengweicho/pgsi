...
def index(request):
	...
	return render(request, "albumapp/index.html", locals())
...
def albumshow(request, albumid=None):  #顯示相簿
	...
	return render(request, "albumapp/albumshow.html", locals())
...
def albumphoto(request, photoid=None, albumid=None):  #顯示單張相片
	...
	photo.phit += 1  #點擊數加1
	photo.save()  #儲存資料
	return render(request, "albumapp/albumphoto.html", locals())
	...
	if request.method == 'POST':  #如果是以POST方式才處理
		...
		if user1 is not None:  #驗證通過
			if user1.is_active:  #帳號有效
				...
				return redirect('/albumapp/adminmain/')  #開啟管理頁面
			else:  #帳號無效
				message = '帳號尚未啟用！'
		else:  #驗證未通過
			message = '登入失敗！'
	return render(request, "albumapp/login.html", locals())

def logout(request):  #登出
	auth.logout(request)
	return redirect('/albumapp/index/')
...
def adminmain(request, albumid=None):  #管理頁面
	if albumid == None:  #按相簿管理鈕進管理頁面
		...
		album.delete()  #移除相簿
		return redirect('/albumapp/adminmain/')
	return render(request, "albumapp/adminmain.html", locals())
...
def adminadd(request):  #新增相簿
	...
	if title=='':  #按新增相簿鈕進入此頁
		...
		unit.save()
		return redirect('/albumapp/adminmain/')
	return render(request, "albumapp/adminadd.html", locals())
		...
		if photoid == '999999':  #按更新及上傳資料鈕
			...
			for upfile in files:
				if upfile != '' and descs[i] != '':
					...
					unit.save()
				i += 1
			return redirect('/albumapp/adminfix/' + str(album.id) + '/')
			...
			photo.delete()  #從資料庫移除
			return redirect('/albumapp/adminfix/' + str(album.id) + '/')
	return render(request, "albumapp/adminfix.html", locals())
