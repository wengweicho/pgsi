...
def upload(request):
    if request.method == 'GET':
        ...
        if not dirlist:
            ...
            dirpath = os.path.join(BASE_DIR,"kmtiger")#kmtiger should be inpuable,TBC
            try:
                os.makedirs(dirpath)
            except OSError as e:
                if e.errno == 17:
                # Dir already exists. No biggie.z
                    pass
            for file in dirlist:
                #{'1.py': 'try1/1.py', '2.py': 'try1/2.py', '3.py': 'try1/try2/3.py', '4.py': 'try1/try2/4.py'}
                position = dirpath+'/'+dict_file_path_pairs.get(str(file))
                """
                    # 获取上传文件的文件名，并将其存储到指定位置
                with open(position, 'wb+')  as destination:     # 打开存储文件
                    for chunk in file.chunks():          # 分块写入文件
                        destination.write(chunk)
                                   #写入完成后关闭文件 At the moment,either the
                """
