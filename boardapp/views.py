...
def index(request, pageindex=None):  #首頁
	...
	return render(request, "boardapp/index.html", locals())
...
def post(request):  #新增留言
	if request.method == "POST":  #如果是以POST方式才處理
		...
		if postform.is_valid():  #通過forms驗證
		  ...
		  unit = models.BoardUnit.objects.create(bname=name, bsubject=subject, bcontent=content, bresponse='')  #新增資料記錄
		  unit.save()  #寫入資料庫
		  message = '已儲存...'
		...
		postform = forms.PostForm()
	return render(request, "boardapp/post.html", locals())
	...
	if request.method == 'POST':  #如果是以POST方式才處理
		...
		if user1 is not None:  #驗證通過
			if user1.is_active:  #帳號有效
				...
				return redirect('/boardapp/adminmain/')  #開啟管理頁面
			else:  #帳號無效
				message = '帳號尚未啟用！'
		else:  #驗證未通過
			message = '登入失敗！'
	return render(request, "boardapp/login.html", locals())
...
def adminmain(request, pageindex=None):  #管理頁面
	...
	if pageindex==None:
		...
		unit.save()  #寫入資料庫
		return redirect('/boardapp/adminmain/ret/')  #返回管理頁面,參數為ret
	currentpage = page+1
	return render(request, "boardapp/adminmain.html", locals())
...
def delete(request, boardid=None, deletetype=None):  #刪除資料
	...
	if deletetype == 'del':  #按確定刪除鈕
		unit.delete()
		return redirect('/boardapp/adminmain/')
	return render(request, "boardapp/delete.html", locals())
